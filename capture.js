...
let _chunks=[],_offBuf=[],_hasSys=false,_analyser=null,_firstChunk=null;
...
async function doStart(){
  ...
  try{
    ...
    _chunks=[];_offBuf=[];_firstChunk=null;
...
/* Convert audio blob to 16kHz mono WAV for reliable Whisper decoding */
async function blobToWav(blob){
  const arrayBuf=await blob.arrayBuffer();
  /* Decode compressed audio (webm/ogg) → PCM */
  ...
  ws(0,'RIFF');view.setUint32(4,36+f32.length*2,true);ws(8,'WAVE');
  ...
  let off=44;
  for(let i=0;i<f32.length;i++){const s=Math.max(-1,Math.min(1,f32[i]));view.setInt16(off,s<0?s*0x8000:s*0x7FFF,true);off+=2;}
...
async function runWhisper(chunks){
  ...
  try{
    ...
    /* Prepend first chunk (WebM header) so fragments decode correctly */
    const allChunks=(_firstChunk&&chunks[0]!==_firstChunk)?[_firstChunk,...chunks]:chunks;
    const rawBlob=new Blob(allChunks,{type:'audio/webm'});
    /* Convert to WAV for reliable Whisper decoding */
    const wavBlob=await blobToWav(rawBlob);
    const url=URL.createObjectURL(wavBlob);
    const result=await _whisper(url);
    URL.revokeObjectURL(url);
    const text=(result&&result.text||'').trim().replace(/^\[BLANK_AUDIO\]$/i,'').replace(/^\[.*\]$/,'');
    if(text){
    ...
    setMsg('⚠ Decode error — retrying next cycle: '+e.message.slice(0,40));
    /* Don't crash — just skip this chunk and continue */
  }
}
...
function doResume(){
  ...
  else if(_mode==='whisper')startOfflineLoop();
